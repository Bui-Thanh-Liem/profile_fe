...
export async function sendMailAdmin(
  ...
  const response = await callApiServerCookie<ISendMail>({
    url: `${process.env.SERVER_HOST}/api/v1/${CONSTANT_ROUTE.MAIL.ROOT}/${CONSTANT_ROUTE.MAIL.SEND_ADMIN}`,
    options: {
      method: "POST",
...
export async function sendMailUser(payload: Partial<ISendMail>) {
  const response = await callApiServerCookie<ISendMail>({
    url: `${process.env.SERVER_HOST}/api/v1/${CONSTANT_ROUTE.MAIL.ROOT}/${CONSTANT_ROUTE.MAIL.SEND_USER}`,
    options: {
      method: "POST",
      body: JSON.stringify(payload),
    },
...
export async function sendMailCustomer(payload: Partial<ISendMail>) {
  const response = await callApiServerCookie<ISendMail>({
    url: `${process.env.SERVER_HOST}/api/v1/${CONSTANT_ROUTE.MAIL.ROOT}/${CONSTANT_ROUTE.MAIL.SEND_CUSTOMER}`,
    options: {
      method: "POST",
      body: JSON.stringify(payload),
    },
...
export async function sendMailBulk(payload: Partial<ISendMail>) {
  const response = await callApiServerCookie<ISendMail>({
    url: `${process.env.SERVER_HOST}/api/v1/${CONSTANT_ROUTE.MAIL.ROOT}/${CONSTANT_ROUTE.MAIL.SEND_BULK}`,
    options: {
      method: "POST",
      body: JSON.stringify(payload),
    },
